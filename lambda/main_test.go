...
func TestHandleRequest_CloudWatchAvailabilityEvent_AppointmentWithinLatestDate(t *testing.T) {
	...
	ntfyServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "SF Enrollment Center slot open", payload["title"])
		assert.Contains(t, payload["message"], appointmentTime.In(time.FixedZone("PDT", -7*3600)).Format("Mon, Jan 2 at 3:04 PM MST"))
		assert.Contains(t, payload["message"], "book now")
		assert.Equal(t, "https://ttp.dhs.gov/schedulerui/", payload["click"])
...
func TestHandleRequest_CloudWatchAvailabilityEvent_MultipleScenarios(t *testing.T) {
	...
	ntfyServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "SF Enrollment Center slot open", payload["title"])
		assert.Contains(t, payload["message"], withinRangeTime.In(time.FixedZone("PDT", -7*3600)).Format("Mon, Jan 2 at 3:04 PM MST"))
		assert.Contains(t, payload["message"], "book now")
		assert.Equal(t, "https://ttp.dhs.gov/schedulerui/", payload["click"])
		w.WriteHeader(http.StatusOK)
	}))
