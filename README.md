...
Generate a VAPID key pair (install [web-push](https://www.npmjs.com/package/web-push) globally or use `npx web-push generate-vapid-keys`). Set `VAPID_SUBJECT` to a `mailto:` or `https:` contact URI for the push service. If `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` are omitted, the API still works for **ntfy-only** subscriptions; browser push is disabled until both keys are set.
